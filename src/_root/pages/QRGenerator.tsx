...
const QRGenerator = () => {
    ...
    const generateQRCode = async (values) => {
        ...
        try {
            ...
            QRCode.toDataURL(values.text, { width: 1000, margin: 2 }, (err, url) => {
                if (!err) {
                    setQrCodePng(url);
                }
            });
            ...
            QRCode.toString(values.text, { type: 'svg' }, (err, string) => {
                if (!err) {
                    setQrCodeSvg(string);
                                ...
                                {qrCodePng && (
                                    <div className="flex flex-col items-center">
                                        <div id="qr-code-container" className="mb-5">
                                            <img src={qrCodePng} alt="QR Code" className="w-64 h-64" />
                                        </div>
                                        {isDownloadVisible && (
